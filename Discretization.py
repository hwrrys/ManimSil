...
class PathScene(Scene):
    CONFIG = {
    "x_coords":[0, -1, -1, -2, -2, -2, -3, -3, -2, -1, 0, 0, 1, 1,2 ,2, 3, 3, 2, 2, 1, 0 , 0],
        "y_coords":[3, 3, 2, 2, 1, 0, 0, -1, -1, -1, -1, -2, -2, -1, -1, 0, 0, 1, 1,2 ,2 ,2, 3]
    }
    """
...
class TransformPathStyle(PathScene):
    def construct(self):
        Reference_text=TextMobject("your fine is 1000 $\\$$")
        dollar = TexMobject()
        path = VMobject()
        path.set_points_smoothly([*[coord(x,y) for x,y in self.tuples]])
        self.add(path)
        self.play(path.make_jagged, run_time=5)
        drunk = Dot()
        drunk1 = Dot()
        self.wait()
        self.play(ApplyMethod(drunk.shift,LEFT))
        self.wait()
        self.play(ApplyMethod(drunk.shift,UP))
        self.wait()
        self.play(ApplyMethod(drunk.shift,RIGHT*2))
        self.wait()
        self.play(ApplyMethod(drunk.shift,UP))
        self.wait()
        self.play(FadeIn(Reference_text.next_to(drunk, UP, buff = 1)))
        self.wait(3)
        Reference_text=TextMobject("your fine is 700 $\\$$")
        drunk2 = Dot()
        drunk3 = Dot()
        self.wait()
        self.play(ReplacementTransform(drunk3, drunk2.shift(LEFT)))
        self.remove(drunk2)
        drunk3.move_to(LEFT)
        self.add(drunk3)
        self.wait()
        self.play(ReplacementTransform(drunk3, drunk2.shift(DOWN)))
        drunk3.move_to(DOWN)
        self.wait()
        self.play(FadeIn(Reference_text.next_to(drunk2, DOWN, buff = 1)))
        self.wait(3)
        """
        There are 3 methods:
            path.make_smooth()
